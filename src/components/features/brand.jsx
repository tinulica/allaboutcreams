...
function Brand( props ) {
    const { adClass = "mt-5 mb-5 owl-simple", count = 100, start = 0, sliderOption = brandSlider } = props;
            ...
            { data.brands.default.slice( start, start + count ).map( ( vari, i ) => {
                return (
                    <Link to="#" className="brand mr-0" key={ i } >
                        <img src={ process.env.PUBLIC_URL + '/' + vari.image } alt={ vari.name } width="50" height="50" />
                    </Link>
                )
            } )
            }
